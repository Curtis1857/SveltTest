        ...
        public async Task RunAsync(Action<string> output = null, int timeout = 2000)
        {
            lock (signal)
            {
                if (process == null)
                {
                    ...
                    //    //UseShellExecute = false,
                    //    //WorkingDirectory = $@"C:\Users\curtis.MAIN\source\repos\SveltTest\SveltTest\"
                    ...
                    //process.StandardInput.WriteLine($" run {scriptName}");
                    process.StandardInput.WriteLine($"npm run {scriptName}");
